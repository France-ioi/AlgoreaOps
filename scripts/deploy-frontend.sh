...
OVERRIDE_FILES=$(yq '.frontend.override_files' ${ENV_FILE})
BUILD_FR=$(yq '.frontend.build_fr' ${ENV_FILE})
SCRIPT_PWD=$(pwd)
...
# Build
test "${OVERRIDE_FILES}" != "null" && cp -r ${ENV_DIR}/${OVERRIDE_FILES} ${APP_BUILD_DIR}/
cd ${APP_BUILD_DIR}
npm install
