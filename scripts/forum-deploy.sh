...
for DEPLOYED_ENV in $(yq '.forum | keys | join(" ")' ${DEPLOYMENTS}); do
  ...
  DEPLOY_DIR=${DEPLOYED_ENV}/${VERSION}-$(./scripts/dir-hash.sh ./environments/forum/${DEPLOYED_ENV})
  LAMBDA_VERSION=$(curl -L --fail https://alg-ops.s3.eu-west-3.amazonaws.com/deployments/forum/${DEPLOY_DIR}/LAMBDA_VERSION || echo "n/a")
