...
yq '.domains[0].allUsersGroup=strenv(ALLUSERSGRP),.domains[0].tempUsersGroup=strenv(TMPUSERSGRP)' ${ENV_DIR}/configs/backend-empty.yaml > ${BUILD_DIR}/conf/config.yaml
cd ${BUILD_DIR}
